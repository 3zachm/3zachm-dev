...
async function getChannelBadges(redis: RedisInstance, channelID: string) {
  return await applyCache(
    redis,
    "TWITCH.CHANNEL_BADGES_" + channelID,
    `https://api.twitch.tv/helix/chat/badges?broadcaster_id=${channelID}`,
    600
  );
}

async function getGlobalBadges(redis: RedisInstance) {
  return await applyCache(
    redis,
    "TWITCH.GLOBAL_BADGES",
    `https://api.twitch.tv/helix/chat/badges/global`,
    3600
  );
}

async function getVideos(redis: RedisInstance, channelID: string) {
  return await applyCache(
    redis,
    "TWITCH.VIDEOS_" + channelID,
    `https://api.twitch.tv/helix/videos?user_id=${channelID}?limit=100`,
    360
  );
}

export { getUserByName, getUserByID, getGlobalEmotes, getChannelEmotes, getChannelBadges, getGlobalBadges, getVideos };
