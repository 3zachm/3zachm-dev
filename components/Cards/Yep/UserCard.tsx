...
function UserCard() {
    ...
    return (
        <div className="flex flex-col max-w-[600px] min-w-[30vw] bg-black rounded-md p-10 z-[10] shadow-lg backdrop-blur bg-opacity-40 justify-center">
            <Text small className="text-white">
                    There have been
            </Text>
            <div className="w-100 flex flex-row items-baseline justify-start">
                <Text h1 size={90} css={{ textGradient: '45deg, $purple500 -20%, $pink300 100%' }} weight="bold" className="text-white pr-4">
                    {(content == "" ) ? "Loading": <CountUp end={data.yep} separator="," duration={2.0} easingFn={easeOutElastic}/>}
                </Text>
                <Text small className="text-white">
                    yeps
                </Text>
            ...
            <div className="w-100 flex flex-row items-baseline justify-start">
                <Text h1 size={70} css={{ textGradient: '45deg, $purple500 -20%, $pink300 100%' }} weight="bold" className="text-white pr-4">
                    {(content == "" ) ? "Loading": <CountUp end={data.cock} separator="," duration={2.0} easingFn={easeOutElastic}/>}
                </Text>
                <Text small className="text-white">
                    🐔&apos;s from 
                </Text>
            ...
            <div className="w-100 flex flex-row items-baseline justify-start">
                <Text h1 size={50} css={{ textGradient: '45deg, $purple500 -20%, $pink300 100%' }} weight="bold" className="text-white pr-4">
                    {(content == "" ) ? "Loading": <CountUp end={data.count} separator="," duration={2.0} easingFn={easeOutElastic}/>}
                </Text>
                <Text small className="text-white">
